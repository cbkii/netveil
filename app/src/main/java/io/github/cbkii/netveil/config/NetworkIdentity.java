...
public final class NetworkIdentity {
     ...
     * An explicit route is created only when an IP has exactly one compatible gateway;
     * ambiguous or unmatched IPs become route-hidden identities instead of guessing.
     */
    public static List<NetworkIdentity> migrateLegacy(List<String> ips,
        ...
        for (String rawIp : ips) {
            if (!Ipv4.isLiteral(rawIp)) continue;
            String ip = Ipv4.canonical(rawIp);
            List<String> compatible = new ArrayList<>();
            if (gateways != null) {
                for (String rawGateway : gateways) {
                    if (!Ipv4.isLiteral(rawGateway)) continue;
                    String candidate = Ipv4.canonical(rawGateway);
            ...
            NetworkIdentity identity = compatible.size() == 1
